    ...
    function populate(argv: string[], context: ParsedContext) {
        ...
        for (const option of options) {
            ...
            if (
                ...
                (!option.conflicts || context.options[option.conflicts] === undefined)
            ) {
                for (const target of targets) {
                    if (target[option.key] !== undefined) {
                        continue
            ...
            if (option.required && !option.noErrors) {
                for (const target of targets) {
                    const requiredWhen: string | undefined =
                        typeof option.required === 'object' && 'when' in option.required
                            ? option.required.when
                            : undefined
                    const requiredUnless: string | undefined =
                        typeof option.required === 'object' && 'unless' in option.required
        ...
        parse: async (argv: string[]) => {
            ...
            if (context.group && !context.command) {
                if (argv.join(' ') === context.group.fullPath) {
                    ...
                    return handleError(
                        context,
                        'Not a valid command in group [' + context.group.key + ']: ' + argv[depth]
                    )
                }
            }
